...
class Book(MyGameObject):
    ...
    def onDraw(self, app: "TheApp"):
        ...
        canvas.draw_text_center(
            self.title,
            self.p + Vector2D(0, -yohaku),
            font=f"{self.size}px serif",
        ...
        self.deleted = not self.p.is_in_rect(
            Rect(Vector2D(0, -yohaku), Vector2D(canvas.width, canvas.height + yohaku * 2))
        )
